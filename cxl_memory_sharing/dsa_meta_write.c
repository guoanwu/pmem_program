...
int main(int argc, char **argv)
{
    ...
    if (argc!= 2 ) {
        ...
        printf("example: ./dsa_meta_write /mnt/famfs/test_file");
        return 0;
    }
    ...
    // the meta data is 64 bytes
    // if it is 0x00, it means it can be write and can't be read
    // it is it 0x10, it means it can be read and can't be write
    ...
    // prepare total_size dram with the data
    while (1) {
	 ...
	 if (write_flag == 0x00) {
		printf("\n prepare the data in dram to write to share file test_file_dsa\n");
	 	printMemory(dram_source+total_size-256, 256);
		gettimeofday(&startVal, NULL);
		dsa_move_data(jobs, DSA_JOB_NUM, dram_source, data_addr, total_size, DSA_MOVE_BLOCK);
		memset(addr,0x10,64);
    		cflush(addr, 64);
